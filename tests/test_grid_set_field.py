"""Tests for Grid.set_field and grid-slot component positioning."""
...
def test_set_field_updates_value() -> None:
    grid = LayoutGrid()
    grid.set_field("body", "world")
    assert grid.body == "world"


def test_set_field_updates_field_metadata() -> None:
    grid = LayoutGrid()
    grid.set_field("body", visible=False, color="red")
...
def test_set_field_metadata_only() -> None:
    grid = LayoutGrid()
    grid.set_field("body", value=UNSET, border="rounded")
...
def test_set_field_accepts_text_component() -> None:
    ...
    grid = TextGrid()
    grid.set_field("body", Text(content="updated"))
