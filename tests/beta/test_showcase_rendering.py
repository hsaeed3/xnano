"""tests.beta.test_showcase_rendering

---

Rendering-quality tests for the beta showcase demo. These guard the traits
...
def test_bench_showcase_tick_and_render(benchmark) -> None:
...
def test_bench_showcase_live_paint(benchmark) -> None:
...
def test_canvas_ir_cache_stays_bounded_under_animation() -> None:
    ...
    from xnano.beta.core import rendering
...
def test_intro_splash_transitions_after_timeout() -> None:
    ...
    try:
        ...
        runtime.set_root(app)
        runtime.render()
        for _ in range(Demo._INTRO_TICKS + 2):
            runtime.perform(Action.tick(66))
        assert isinstance(app.view, Showcase)
    finally:
        runtime.close()
