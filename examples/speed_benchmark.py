    ...
    class TextualStressApp(App):
        ...
        async def on_mount(self) -> None:
            ...
            while time.time() - start_time < duration:
                ...
                self.query_one("#header", Static).update(f"  Textual Benchmark  ●  FPS: {fps:.1f}")
                self.query_one("#footer", Static).update(f"  Total Frames: {self.frames}")
...
def run_pytermgui_benchmark(duration: float) -> tuple[int, float, float, float]:
...
def run_monitored_benchmark(framework: str, duration: float) -> dict:
    ...
    cmd = [sys.executable, __file__, "--run", framework, "--duration", str(duration)]
    ...
    while proc.poll() is None:
        if p:
            try:
                ...
                mem_samples.append(p.memory_info().rss / (1024 * 1024))  # Convert to MB
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                pass
        time.sleep(0.05)
    ...
    avg_cpu = (sum(cpu_samples) / len(cpu_samples) / num_cores) if cpu_samples else 0.0
...
def main() -> None:
    ...
    if xn["avg_cpu"] < tx["avg_cpu"]:
        ...
        print(f"📉 xnano consumes {cpu_diff:.1f}% less CPU resource than Textual.")
    if xn["peak_mem"] < tx["peak_mem"]:
        mem_saved = tx["peak_mem"] - xn["peak_mem"]
        print(f"🧠 xnano uses {mem_saved:.1f} MB less RAM memory footprint than Textual.")
    print("=" * 105 + "\n")
