...
With no arguments this runs the beta feature showcase: a mosaic of differently
sized boxes, several with their own inner tabs and effects, driven entirely
through ``xnano.beta``. The whole scene stays fluid because every animated box
...
from xnano.beta.components.bar import Bar
...
_INTRO_CLIPS: tuple[str, ...] = ("luffy", "luffy_deck")
"""Precomputed ``.xni`` clips eligible for the intro splash."""

_XNI_DIRECTORY = pathlib.Path(__file__).resolve().parents[2]
"""Package root holding the bundled ``.xni`` clips (``xnano/``)."""
...
def _load_intro_clip() -> Any | None:
    """Decode one bundled intro clip, or ``None`` when unavailable."""
    from xnano.beta.components.image import ImageData

    path = _XNI_DIRECTORY / f"{random.choice(_INTRO_CLIPS)}.xni"
    if not path.is_file():
        return None
    try:
        return ImageData.from_bytes(path.read_bytes())
    except Exception:
        return None
...
class IntroSplash(Component):
    ...
    def compose(self, ctx: ComponentRenderContext) -> Content | None:
        ...
        from xnano.beta.core.rendering import _cell_canvas_ir
...
class Showcase(BaseGrid, direction="vertical", gap=0):
    """The full beta showcase: header, the box mosaic, and a keybind legend."""
...
class Demo(BaseGrid, direction="vertical"):
    ...
    _INTRO_TICKS = 44
    """Splash length in ticks (~3s at the 66ms cadence)."""
    ...
    def _advance(self) -> None:
        ...
        if isinstance(view, IntroSplash):
            ...
            if self._elapsed >= self._INTRO_TICKS:
...
def run_showcase() -> None:
    """Run the beta feature showcase on the live terminal."""
    from xnano.beta.terminal import Terminal

    # ~12fps. A full-screen truecolor repaint is the cost (Python lowering +
    # the emulator redrawing every cell), so the only real lever is painting
    # less often; the drifts are slow enough that 12fps reads fine.
    # ponytail: fixed cap; render-on-change (skip the paint when the buffer is
    # unchanged) is the next step if this still runs hot.
    Terminal(title="xnano · showcase", tick_interval=80).run(Demo())
