...
`Command` is a model-like CLI builder under `xnano.beta.commands`. You declare
a named app, decorate a callback (or subcommands) with options and types, then
...
```python
from xnano.beta.commands import Command, CommandLineParameter, HelpException
```
...
[Web UI](../webui/index.md) cover interactive surfaces. Together they aim for
one declarative style from one-shot CLI tools through full interfaces — still
evolving under `xnano.beta`.
