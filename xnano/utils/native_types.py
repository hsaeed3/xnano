...
(nodes, the terminal controller, `Grid`) only ever deals in `xnano` types.
...
def frame_length_overhead(frame: Frame, direction: Direction) -> int:
    ...
    overhead) and to size non-`Grid` renderables sharing a `GridFieldInfo`'s
    chrome.
