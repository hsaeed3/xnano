...
_PRIORITY_CYCLE: dict[str, Priority] = {"low": "mid", "mid": "high", "high": "low"}
...
_TITLE_POOL = [
    "Add dark mode support", "Write API documentation", "Set up CI pipeline",
    "Refactor auth module", "Benchmark render loop", "Fix memory leak",
    "Audit dependency versions", "Add rate limiting", "Update OpenAPI spec",
    "Profile startup time", "Cache query results", "Add integration tests",
    "Improve error messages", "Extract config service", "Harden input validation",
    "Add webhook support", "Migrate database schema", "Write runbook",
]
    ...
    def get_node(self, ctx: ComponentRenderContext):  # type: ignore[override]
        ...
        for task in self.tasks:
            ...
            rows.append(TableRowItem(cells=[
                TableCellItem(content=SpanNode(
                    content=f" {sym} {task.priority[:3]} ",
                    color=pc, modifiers=["bold"],
                )),
                TableCellItem(content=f"  {task.title}",
        ...
        return TableNode(
            ...
            highlight_background=self.accent_bg or tailwind_color("violet", 700),
    ...
    def get_node(self, ctx: ComponentRenderContext):  # type: ignore[override]
        ...
        for ts, icon, action, col_idx in self.events:
            rows.append(TableRowItem(cells=[
                ...
                TableCellItem(content=SpanNode(
                    content=f" {icon} ", color=_COL_ACCENTS[col_idx], modifiers=["bold"],
                )),
                TableCellItem(content=f" {action}", color=tailwind_color("slate", 700)),
            ], height=1))
...
class VelocityChart(AbstractComponent):
    ...
    def get_node(self, ctx: ComponentRenderContext):  # type: ignore[override]
        ...
        for i, v in enumerate(data):
            if v > 0:
                shapes.append(CanvasLine(x1=float(i), y1=0.0, x2=float(i), y2=float(v), color=area_c))
...
class PriorityBreakdown(AbstractComponent):
    ...
    def get_node(self, ctx: ComponentRenderContext):  # type: ignore[override]
        ...
        groups = [
            BarGroupItem(bars=[BarItem(value=counts[p], label=p[:3], color=_PRIORITY_FG[p])])
            for p in ("high", "mid", "low")
        ]
        return BarChartNode(
            groups=groups,
            bar_width=5, bar_gap=0, group_gap=3,
            max_value=max_v,
            direction="vertical",
...
class SprintStats(Grid, direction="vertical", gap=0):
    completion: SprintGauge = Field(default_factory=SprintGauge, height=3,
                                    border="plain", border_color=tailwind_color("slate", 800))
    in_focus:   SprintGauge = Field(default_factory=SprintGauge, height=3,
                                    border="plain", border_color=tailwind_color("slate", 800))
    high_pri:   SprintGauge = Field(default_factory=SprintGauge, height=3,
                                    border="plain", border_color=tailwind_color("slate", 800))
...
class KanbanApp(Grid, direction="vertical"):
    ...
    task_data:   list = Field(default_factory=lambda: [list(col) for col in _SEED], state=True)
    ...
    def _new_task(self) -> None:
        ...
        task = Task(
            ...
            priority=cast(Priority, random.choices(["high", "mid", "low"], weights=[2, 5, 3])[0]),
    ...
    def _cycle_priority(self) -> None:
        ...
        data[c][idx]     = Task(title=old.title, priority=new_p, tag=old.tag, tid=old.tid)
        self.task_data   = data
    ...
    def grid_render(self) -> None:
        ...
        self.main.left.cards    = TaskList(tasks=data[c], selected=sel[c], accent_bg=_COL_HL_BG[c])
        ...
        sp.high_pri = SprintGauge(
            ...
            progress=danger_pct,
            filled=tailwind_color("rose", 500) if high_open > 3 else tailwind_color("amber", 400),
        )
