...
class LooseGrid(Grid, strict=False):
    count: int = Field(default="not-an-int")
...
def test_strict_init_accepts_any_renderable_value() -> None:
    class Panel(Grid):
        body: Renderable = Field(default=123)
...
def test_pydantic_model_field_rejects_wrong_type() -> None:
    class Bad(ModelGrid):
        model: UserModel = Field(default=123)  # type: ignore[arg-type]
