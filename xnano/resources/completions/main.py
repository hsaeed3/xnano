...
class Completions:
    ...
    # instructor patch
    def instructor_patch(self, mode : CompletionInstructorModeParam = "tool_call") -> None:
        """
    ...
    def instructor_apatch(self, mode : CompletionInstructorModeParam = "tool_call") -> None:
        """
        ...
        if response_model:
            ...
            if isinstance(original_response_model, type) and not issubclass(original_response_model, BaseModel):
                if return_messages:
                    ...
                    if self.verbose:
                        console.message(
                            "✅ [green]Returning messages[/green]"
                        )
                ...
                if self.verbose:
                    console.message(
                        "✅ [green]Successfully patched response[/green]"
                    )
    ...
    def completion(
        ...
        return self.run_completion(
            messages = messages, model = model, context = context, memory = memory, memory_limit = memory_limit,
