...
def test_frame_cannot_be_instantiated_directly():
    with pytest.raises(TypeError, match="Frame instances are created internally"):
        Frame()
...
def test_terminal_management():
    ...
    with mock.patch("xnano._core.Terminal.init", return_value=mock_core_term) as mock_init:
        term = Terminal()
