...
def determine_batch_needed(
    ...
    if determination.multi_message_batch_job and determination.multi_model_batch_job:
        raise XNANOException("Batch jobs both using multiple message threads & multiple models are not supported.")
...
def warn_unsupported_batch_params(
    ...
    if tools:
        for tool in tools:
            if isinstance(tool, str):
                console.warning(f"Batch completions do not support generated tools. Ignoring {tool}...")
                tools.remove(tool)
    ...
    if stream and response_format:
        console.warning("Batch completions do not support streaming when using structured outputs. Ignoring...")

    if memory:
        console.warning("Batch completions do not support using Vector Stores for context. Ignoring...")
...
def create_batch_completion_job(
    ...
    if response_model:
        ...
        print("[bold]Notice[/bold] - Batch completions default to using LiteLLM `response_format` parameter. Instructor will not be used...")
    ...
    if verbose:
        console.message(f"✅ [green]Successfully formatted {len(formatted_messages)} sets of messages.[/green]")
    ...
    if verbose:
        console.message(f"✅ [green]Successfully created batch completion job with {len(messages)} sets of messages.[/green]")
    ...
    if response_format:
        ...
        for response in response:
            formatted_responses.append(build_pydantic_model_from_string_dictionary(response, response_format))
...
def create_batch_completion_job_with_multiple_models(
    ...
    if not isinstance(model, list):
        raise XNANOException("Batch completions with multiple models requires a list of models.")
    ...
    try:
        ...
        raise XNANOException(f"Failed to format messages for multiple model batch completion: {e}")
    
    if verbose:
        console.message(f"✅ [green]Successfully formatted {len(messages)} messages.[/green]")
    ...
    if verbose:
        console.message(f"✅ [green]Successfully created batch completion job with {len(messages)} sets of messages and {len(model)} models.[/green]")
