...
draws to a terminal screen through `xnano_core`. Every `Grid` and every
...
`AbstractController`'s contract instead of a parallel one `Grid` used to
...
from xnano.core.nodes.terminal import AbstractTerminalNode, ParagraphNode
from xnano.types import Alignment, Area, Direction, Size
from xnano.utils import native_types
