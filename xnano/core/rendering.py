"""xnano.core.rendering

---

Lower components and content primitives into ``xnano_core`` render nodes.
"""
...
def _line_from_runs(runs: tuple[Run, ...]) -> core.IrLine:
    """Build one renderer line from styled runs."""
...
def _table_row(row: TableRow) -> tuple[list[Any], Any, Any, int]:
    """Convert a table row for render IR."""
    cells = []
...
def _plot_node(content: Plot) -> core.CoreRenderNode:
    """Lower a plot through the native chart widget."""
    datasets = []
...
def lower_content(content: Any) -> core.CoreRenderNode:
    """Lower a renderable into one native render node.
