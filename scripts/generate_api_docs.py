        ...
        if isinstance(value, str):
            ...
            for sub_item in value:
                formatted_items.append(
                    " " * inner_indent
                    + pretty_format_navigation(sub_item, inner_indent)
                )
...
def replace_navigation_in_toml(
    toml_content: str, new_navigation_string: str
) -> str:
...
def get_module_path(
    base_package_name: str, relative_path: pathlib.Path
) -> str:
...
def get_navigation_tree(
    directory_path: pathlib.Path, docs_directory: pathlib.Path
) -> List[Any]:
...
def generate_api_documentation() -> None:
    ...
    base_directory = pathlib.Path(
        "/Users/hammad/Development/xnano/xnano-main/xnano-docs"
    )
    ...
    targets = [
        ...
        (
            "xnano_core",
            base_directory / "xnano-core" / "python" / "xnano_core",
            api_directory / "xnano-core",
        ),
    ]
        ...
        for root, _, files in os.walk(source_directory):
            for file_name in files:
                if file_name.endswith(".py"):
                    ...
                    relative_path = python_file_path.relative_to(
                        source_directory
                    )
                    ...
                    if file_name == "__main__.py":
                        ...
                        if is_top_level:
                            ...
                            if package_name == "xnano":
                                markdown_file_path = (
                                    target_directory / "xnano.md"
                                )
                            else:
                                markdown_file_path = (
                                    target_directory / "xnano-core.md"
                                )
                        ...
                        # Standard module
                        markdown_file_path = (
                            target_directory / relative_path.with_suffix(".md")
                        )
                    ...
                    markdown_file_path.parent.mkdir(
                    ...
                    if not markdown_file_path.exists():
                        with open(markdown_file_path, "w") as f:
                            f.write(content)
                        print(
                            f"Created: {markdown_file_path.relative_to(base_directory)}"
                        )
                        created_count += 1
                    else:
                        with open(markdown_file_path, "w") as f:
    ...
    api_reference_navigation = get_navigation_tree(
        api_directory, docs_directory
    )

    api_reference_item = {"API Reference": api_reference_navigation}

    new_navigation.append(api_reference_item)

    # Format the new nav array using our custom pretty formatter
    new_navigation_string = "nav = " + pretty_format_navigation(
        new_navigation, indent=0
    )

    # Replace navigation section in the TOML file content
    updated_toml_content = replace_navigation_in_toml(
        toml_content, new_navigation_string
    )
