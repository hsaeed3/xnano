...
def _get_renderable_annotation() -> Any:
    ...
    if _RENDERABLE_ANNOTATION is None:
        ...
        _RENDERABLE_ANNOTATION = (
            str | Component | BaseGrid | collections.abc.Sequence[Any]
        )
...
def _is_component_type(annotation: Any) -> bool:
    """Return whether ``annotation`` identifies a component class."""
    if not inspect.isclass(annotation):
        return False
    return bool(getattr(annotation, "_xnano_component_base", False))
